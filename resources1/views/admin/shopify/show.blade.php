                    ...
                    <table class="dt-scrollableTable table table-bordered">
                        <thead>
                            <tr>
                                ...
                                <th>{{ __('admin.phone_no') }}/ {{__('admin.email')}}</th>
                                <th>{{ __('admin.address') }}</th>
                                <th>{{ __('admin.city') }}</th>
                                <th>{{ __('admin.emirate') }}</th>
                        ...
                        @foreach ($data as $order)
                            <tr>
                                ...
                                <td><span class="text-danger">{{$order->financial_status}}</span></td>
                                <td>
                                    @php $shipment = App\Models\Shipment::where('shopify_order_id',$order->order_id)->first(); @endphp
                                    @if ($shipment && $shipment->status_id == 3)
                                        <span class="text-success">done</span>
                                    @else
                                    <a id="close" class="btn btn-sm btn-label-info close"
                                    data-url="{{ route('admin.shopify_order_update', [$order->order_id, $order->order_number]) }}">{{ __('admin.close') }}</a>
                                    @endif


                                    @if ($shipment)
                                    @else
                                        <a href="{{ route('admin.shipments_create_shopify', [$order->order_id]) }}"
                                            class="btn btn-sm btn-label-info">{{ __('admin.shipments_create_shipment') }}</a>
                                    @endif
