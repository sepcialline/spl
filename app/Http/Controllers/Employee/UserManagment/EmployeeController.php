...
class EmployeeController extends Controller
{
    ...
    {
        $this->middleware(['permission:employee-Employee-show-page,employee'], ['only' => ['index']]);
        $this->middleware(['permission:employee-Employee-add,employee'], ['only' => ['create','store']]);
        $this->middleware(['permission:employee-Employee-show,employee'], ['only' => ['show']]);
        $this->middleware(['permission:employee-Employee-edit,employee'], ['only' => ['edit','updateEmployee']]);
        $this->middleware(['permission:employee-Employee-change-status,employee'], ['only' => ['update_status']]);
        $this->middleware(['permission:employee-Employee-delete,employee'], ['only' => ['destroy']]);
    ...
    public function create()
    {
        ...
        return view('employee.users.employee.create', compact('emirates', 'branches', 'cities', 'roles','departments'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $employee->password = bcrypt($request->employee_password);
        ...
        $employee->branch_id = $request->employee_branch;
    ...
    public function edit(string $id)
    {
        ...
        $roles = Role::where('guard_name', 'employee')->get();
        $values = array();
        $current_roles = DB::table('model_has_roles')->where('model_type','App\\Models\\Employee')->where('model_id', $id)->get();
        foreach ($current_roles as $current_role) {
            # code...
            $role = Role::query()->where('id', $current_role->role_id)->get()[0];
            array_push($values, $role->name);
        }
        return view('employee.users.employee.update', compact('emirates', 'branches', 'cities', 'roles', 'data', 'values','departments'));
    }
