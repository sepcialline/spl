<x-vendor-layout>
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="card p-3  mb-4">
            ...
            <form method="POST" class=" needs-validation" novalidate
                action="{{ route('vendor.company_profile_update') }}" enctype="multipart/form-data" class="card-body">
                @csrf
                <div class="row pt-3  g-3 ">
                    ...
                    <div class="col-md-3">
                        ...
                        <input required type="hidden" name="vendor_rate" id="vendor_rate" class="form-control"
                            placeholder="{{ __('admin.vendors_companies_company_vendor_rate') }}"
                            value="{{ $company->vendor_rate }}" />

                        <input required type="text" name="" id="" class="form-control" readonly
                            disabled placeholder="{{ __('admin.vendors_companies_company_vendor_rate') }}"
                            value="{{ $company->vendor_rate }}" />
                        @error('vendor_rate')
                            <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
                        ...
                        <select name="city_id" id="city_id" class="js-example-basic-single form-select" required>
                            @foreach ($cities as $city)
                                <option value="{{ $city->id }}"
                                    {{ $company->city_id == $city->id ? 'selected' : '' }}>{{ $city->name }}
                                </option>
                            @endforeach
                        </select>
                        @error('city_id')
                ...
                <div class="row">
                    <div class="col">
                        ...
                        <input type="tel" value="{{ $company?->phone_number }}" name="phone_number"
                            class="form-control phone" id="phone_number">
                    </div>
                    <div class="col">
                        <label class="form-label" for="mobile_number">{{ __('admin.mobile') }}</label>
                        <input type="tel" name="mobile_number" value="{{ $company?->mobile_number }}"
                            class="form-control phone" id="mobile_number">
                ...
                <div class="row  g-3">
                    <div class="col-md-6">
                        ...
                        <div class="col-md-4 my-4">
                            <div class="avatar avatar-xl mb-3">
                                @if ($company->logo)
                                <img id="avatar"
                                    src="{{ asset('build/assets/img/uploads/logos/' . $company->logo . '') }}"
                                    alt="{{ __('admin.user_management_admin_avatar') }}">
                                @else
                                <img id="avatar"
                                src="{{ asset('build/assets/img/uploads/avatars/1.png') }}"
                                alt="{{ __('admin.user_management_admin_avatar') }}">
                                @endif
                            </div>
                            <label for="logo"
                        ...
                        <select class="js-example-basic-single" name="vendor">
                            ...
                            @foreach ($vendors as $vendor)
                                <option value="{{ $vendor->id }}"
                                    {{ $company->vendor_id == $vendor->id ? 'selected' : '' }}>{{ $vendor->name }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="row">
                        ...
                        @if (count($company->bankAccounts) > 0)
                            <table class='table table-active'>
                                ...
                                <tbody>
                                    @foreach ($company->bankAccounts as $bankAccount)
                                        <input type="hidden" name="banks_name[]" value={{ $bankAccount->bank_id }}>
                                        <input type="hidden" name="ibans[]" value={{ $bankAccount->iban_number }}>
                                        <input type="hidden" name="name_owner[]"
                                            value={{ $bankAccount->name_owner }}>
                                        <tr>
                                            <td>{{ $bankAccount?->bank?->name_bank }}</td>
                                            <td>{{ $bankAccount?->iban_number }}</td>
                                            <td>{{ $bankAccount?->name_owner }}</td>
                                            <td>
                                                <a href="{{ route('vendor.deleteAccountBank', $bankAccount->id) }}"
                        ...
                        <div id="bankFields">
                            <div class="row mb-3 bank-entry">
                                ...
                                <div class="col">
                                    <label for="name_owner" class="form-label">{{ __('admin.name_owner') }}</label>
                                    <input type="text" name="name_owner[]" class="form-control" placeholder="">
                                </div>
                                <div class="col">
    ...
    @section('VendorsJS')
        ...
        <script type="text/javascript">
            var lng = 55.513641;
            $('#map_latitude').val({{ $company->latitude }});
            var lat = 25.405216;
            $('#map_longitude').val({{ $company->longitude }});
        ...
        <div class="col">
            <label for="iban" class="form-label">{{ __('admin.name_owner') }}</label>
            <input type="text" name="name_owner[]" class="form-control" placeholder="" required>
        </div>
        <div class="col">
