<x-app-layout>
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="card p-3  mb-4">
            <form method="POST" class=" was-validated" action="{{ route('admin.recived_shipment_update') }}"
                ...
                <div class="row pt-3  g-3 ">
                        ...
                        <select class="form-select js-example-basic-single" id="rider_id" required
                            ...
                            @foreach ($riders as $rider)
                                <option value={{ $rider->id }}
                                    {{($rider->id == $shipment->rider_id ? 'selected' : '')}}>{{ $rider->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-3 col-md-4">
                        ...
                        <select class="form-select js-example-basic-single" id="vendor_id"
                            ...
                            @foreach ($vendors as $vendor)
                                <option value={{ $vendor->id }}
                                    {{($vendor->id == $shipment->vendor_id ? 'selected' : '')}}>{{ $vendor->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-3 col-md-4">
                            <label for="vendor_id" class="form-label">تاجر اخر</label>
                        <input type="text" value="{{$shipment->vendor_if_not_in_system}}" class="form-control" name="vendor_if_not_in_system" id="vendor_if_not_in_system">
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input required type="date" name="date" id="date" class="form-control" value="{{\Carbon\Carbon::parse($shipment->date)->format('Y-m-d')}}"
                    ...
                    <div class="col-md-6">
                        ...
                        <input required type="text" name="count_of_shipments" id="count_of_shipments" class="form-control"
                            value="{{$shipment->count_of_shipments }}" />
                    </div>
                    <div class="col-md-12">
                        <div class="avatar avatar-xl mb-3">
                            <img id="avatar" src="{{ asset('build/assets/img/uploads/documents/'.$shipment->image) }}"
