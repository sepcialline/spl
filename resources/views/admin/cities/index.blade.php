    ...
    <div class="container-xxl flex-grow-1 container-p-y ">
        ...
        <div class="card p-4">
            <div class="d-flex justify-content-sm-between align-items-sm-center">
                <h5 class="card-header"> {{ __('admin.city_city_list') }}</h5>

                @can('admin-City-add')
                    <a href="{{ route('admin.city_create') }}"
                        class="btn btn-label-secondary">{{ __('admin.city_city_add') }}</a>
                @endcan
            ...
            <div class="card-datatable table-responsive text-nowrap px-2 py-2">
                <table class="dt-scrollableTable table table-bordered">
                    <thead>
                        <tr>
                            <th>{{ __('admin.city_city_name') }}</th>
                            <th style="width:70%">{{ __('admin.city_city_emirate') }}</th>
                    ...
                    @foreach ($data as $row)
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                @endcan
                                @can('admin-City-delete')
                                    <a id="delete" class="btn btn-label-danger delete"
                                        data-url="{{ route('admin.city_delete', $row['id']) }}">{{ __('admin.branch_action_delete') }}</a>
                                @endcan
                            </td>
                        </tr>
                    @endforeach
