<x-app-layout>
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="card p-3  mb-4">
            <form method="POST" class=" was-validated" action="{{ route('admin.expenses_update')}}"
                ...
                <div class="row pt-3  g-3 ">
                    ...
                    <div class="mb-3 col-md-6">
                        ...
                        <select class="form-select js-example-basic-single" id="rider_id" required
                            ...
                            @foreach ($riders as $rider)
                                <option value={{ $rider->id }} {{($rider->id == $expense->rider_id) ? 'selected' : ''}}>{{ $rider->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-3 col-md-6">
                        ...
                        <select class="form-select js-example-basic-single" required id="car_plate_id"
                            ...
                            @foreach ($car_plates as $car)
                                <option value={{ $car->id }} {{($car->id == $expense->car_plate) ? 'selected' : ''}}>{{ $car->car_name }} # {{ $car->car_plate }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-3 col-md-6">
                        ...
                        <select class="form-select js-example-basic-single" id="expense_type_id" required aria-label="Default select example"
                            ...
                            @foreach ($expense_types as $expense_type)
                                <option value={{ $expense_type->id }} {{($expense_type->id == $expense->expense_type) ? 'selected' : ''}}>{{ $expense_type->name }}<option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-3 col-md-6">
                        ...
                        <select class="form-select js-example-basic-single" id="payment_type_id" required aria-label="Default select example"
                            ...
                            @foreach ($payment_types as $payment_type)
                                <option value={{ $payment_type->id }} {{($payment_type->id == $expense->payment_type) ? 'selected' : ''}}>{{ $payment_type->name }}<option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input required type="date" name="date" value="{{\Carbon\Carbon::parse($expense->date)->format('Y-m-d')}}" id="date" class="form-control"
                    ...
                    <div class="col-md-12">
                        <div class="avatar avatar-xl mb-3">
                            <img id="avatar" src="{{ asset('build/assets/img/uploads/documents/'.$expense->photo) }}"
                                alt="{{ __('admin.dashboard_expenses') }}">
                        </div>
