<x-app-layout>
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="card my-2">
            <div class="card-body">
                <h2> تسديد بالجملة</h2>
                <form action="{{ route('admin.account.bulk_payment') }}" method="post" class="was-validated">
                    @csrf
                    <div class="row">
                        <div class="col">
                            <label for="bulk_company_id">{{ __('admin.company_list') }}</label>
                            <select id="select2Multiple" class="select2 form-select" name="bulk_company_id[]" multiple
                                required>
                        ...
                        <div class="col">
                            <label for="till_date">{{ __('admin.to') }}</label>
                            <input type="date" name="till_date" class="form-control" required>
                        </div>
                    </div>
                    <button type="submit" class="btn btn-label-danger mt-2">تسديد</button>
                </form>
                        ...
                        <div class="col-md col-12 mb-4">
                            ...
                            <div class="input-group input-daterange" id="bs-datepicker-daterange">
                                ...
                                <input type="date" name="date_from" id="dateRangePicker" placeholder="MM/DD/YYYY"
                                    ...
                                    <label for="select2Multiple" class="form-label">{{ __('admin.to') }}</label>
                        ...
                        <tbody>
                            @foreach ($shipments as $shipment)
                                <tr>
                                    ...
                                    <td>{{$shipment->date}}</td>
                                    <td>{{ $shipment->shipment->shipment_no }} <br>
                                        #{{ $shipment->shipment->shipment_refrence }} <br>
                                        {{ $shipment->shipment->Company->name }} <br>
                                        {{ $shipment->shipment->paymentMethod->name }} |
                                        {{ $shipment->shipment->feesType->name }}
                                    </td>
                                    <td>
                                        {{-- @foreach ($shipment->payments as $payment) --}}
                                        {{ $shipment->payment_number }} | {{ $shipment->paymentMethod->name }} :
                                        {{ $shipment->amount }} <br>
                                        {{-- @endforeach --}}
                                    </td>
                                    <td>
    ...
    @section('VendorsJS')
        ...
        <script>
            $(document).on('click', '#collect', function() {
                ...
                $.ajax({
                    method: 'POST',
                    url: "{{ route('admin.transactions_pay_to_the_merchant') }}",
                    data: {
                        payments: payments
                    },
