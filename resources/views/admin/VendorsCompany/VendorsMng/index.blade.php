    ...
    <div class="container-xxl flex-grow-1 container-p-y ">
        ...
        <div class="card p-4">
            <div class="d-flex justify-content-sm-between align-items-sm-center">
                <h5 class="card-header"> {{ __('admin.vendors') }}</h5>
                <div class="form-outline me-2">
                    ...
                    <input type="search" id="form1" class="form-control" placeholder="Type query"
                        aria-label="Search" data-url="{{ route('admin.vendors_search', '') }}" />
                </div>
                @can('admin-Company-add')
                    <a href="{{ route('admin.vendors_create') }}"
                ...
                <table id="table1" class="dt-scrollableTable table table-bordered">
                    <thead>
                        <tr>
                            <th>#</th>
                            <th>{{ __('admin.name') }}</th>
                            <th>{{ __('admin.vendors_companies_list') }}</th>
                    ...
                    <tbody>
                        ...
                        @foreach ($vendors as $vendor)
                            <tr>
                                ...
                                <td>{{ $vendor?->email }} <br> {{ $vendor?->mobile }}</td>
                                <td>
                                    @can('admin-Employees-change-status')
                                        <label class="switch switch-primary">
                                            <input type="checkbox" class="switch-input status"
                                                data-url="{{ route('admin.vendors_update_status', $vendor['id']) }}"
                                                name="status" id="status"
                                                {{ $vendor['status'] ? 'checked' : !'checked' }} />
                                ...
                                <td>
                                    ...
                                    @can('admin-Company-delete')
                                        <a data-url="{{ route('admin.vendors_delete', $vendor->id) }}"
                                            class="btn btn-sm btn-label-danger delete">{{ __('admin.delete') }}</a>
                                    @endcan
                                    @can('admin-Company-show')
                                    <a href="{{ route('admin.vendors_show', $vendor->id) }}"
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            data.results.forEach(function(item, index) {
                                ...
                                output += '<td>' + item.name.en + ' | ' + item.name.ar + '</td>';

                                output += '<td>';
                                if (item.companies.length >= 1) {
                                    item.companies.forEach(function(itm, idx) {
                                        output += itm.name + (idx < item.companies.length -
                                            1 ? ', ' : ''
                                        ); // Add a comma if there are more companies
                                    });
                                } else {
                                    output += 'No companies';
                                }
                                ...
                                output += '<td>' + item.email + '<br>' + item.mobile + '</td>';
                                output += '<td>' +
                                    '<a href="edit/' + item.id +
                                    '" class="btn btn-label-secondary">{{ __('admin.branch_action_edit') }}</a>' +
                                    '<a id="delete" class="btn btn-label-danger delete" data-url="{{ route('admin.vendors_delete', '+ item.id+') }}">{{ __('admin.branch_action_delete') }}</a>' +
                                    '</td>';
                                output += '</tr>';
                            });
