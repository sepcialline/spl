<x-employee-layout>
    ...
    @section('title')
        {{ __('admin.transactions') }} | {{ __('admin.transactions_Withdrawal_from_the_merchant') }}
    @endsection
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <h4 class="py-3 breadcrumb-wrapper mb-4">
            ...
            {{ __('admin.transactions_Withdrawal_from_the_merchant') }}
        </h4>
                        ...
                        <tbody>
                            @foreach ($shipments as $shipment)
                                <tr>
                                    ...
                                    {{ $payment->payment_number }} | {{$payment->paymentMethod->name}} <br>
                                    @endforeach
                                    </td>
                                    <td>
                                        @if ($shipment->is_split_payment == 0)
                                        {{ $shipment->specialline_due }}  <br>
    ...
    @section('VendorsJS')
        ...
        <script>
            $(document).on('click', '#collect', function() {
                ...
                $.ajax({
                    method: 'POST',
                    url: "{{ route('employee.transactions_Withdrawal_from_the_merchant') }}",
                    data: {
                        payments: payments
                    },
