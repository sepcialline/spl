                ...
                <div class="d-flex ">
                    <div class="form-outline me-2">
                        ...
                        <input type="search" id="form1" class="form-control" placeholder="Type query"
                            aria-label="Search" data-url="{{ route('employee.users_rider_search', '') }}" />
                    </div>
                    @if (Auth::guard('employee')->user()->can('employee-rider-add'))
                        <a href="{{ route('employee.users_rider_create') }}"
                        ...
                        <tbody id="original" class="table-body table-border-bottom-0">
                            @foreach ($data as $item)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        {{-- @if (Auth::guard('employee')->user()->can('employee-rider-change-status')) --}}
                                            <label class="switch switch-primary">
                                                <input type="checkbox" class="switch-input status"
                                                    data-url="{{ route('employee.users_rider_update_status', $item['id']) }}"
                                    ...
                                    <td>
                                        @if (Auth::guard('employee')->user()->can('employee-rider-show'))
                                            <a href="{{ $item['id'] }}"
                                                class="btn btn-label-dark">{{ __('admin.branch_action_show') }}</a>
                                        @endif

                                        @if (Auth::guard('employee')->user()->can('employee-rider-edit'))
                                            <a href="edit/{{ $item['id'] }}"
                                                class="btn btn-label-secondary">{{ __('admin.branch_action_edit') }}</a>
                                        @endif

                                        @if (Auth::guard('employee')->user()->can('employee-rider-delete'))
                                            <a id="delete" class="btn btn-label-danger delete"
                                                data-url="{{ route('employee.users_rider_destroy', $item['id']) }}">{{ __('admin.branch_action_delete') }}</a>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
