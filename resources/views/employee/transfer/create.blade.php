<x-employee-layout>
    @section('title')
        ...
        {{ __('admin.warehouse') }} | {{ __('admin.warehouse_transfer_request') }}
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="py-3 breadcrumb-wrapper mb-4">
            ...
            {{ __('admin.warehouse_transfer_request') }}
        </h4>

        <div class="card p-3  mb-4">
            <form method="POST" class=" needs-validation" novalidate action="{{ route('employee.transfer_store') }}"
                enctype="multipart/form-data" class="card-body">
                @csrf
                <div class="row pt-3  g-3 ">
                    <div class="col-md-6">
                        <label for="vendor_id"
                            class="form-label">{{ __('admin.products_vendor_name') }}</label>
                        <select id="vendor_id" name="vendor_id" value="{{ old('vendor_id') }}"
                            class="select2 form-select    @error('vendor_id') is-invalid @enderror"   data-allow-clear="false"
                            aria-label="vendor_id select example">
                    ...
                    <div class="col-md-6">
                        <label for="product_id"
                            class="form-label">{{ __('admin.products_product_name') }}</label>
                        <select id="product_id" name="product_id" value="{{ old('product_id') }}"
                            class="select2 form-select    @error('product_id') is-invalid @enderror"   data-allow-clear="false"
                            aria-label="product_id select example">
                            <option value="">{{ __('admin.please_select') }}</option>
                        </select>
                        @error('product_id')
                            <div class="alert alert-danger">{{ $message }}</div>
                    ...
                    <div class="col-md-12">
                        <label class="form-label"
                            for="quantity">{{ __('admin.warehouse_transfer_quantity') }}</label>
    ...
    @section('VendorsJS')
        ...
        <script>
            $(document).on('change', '#vendor_id', function() {
                ...
                $.ajax({
                    type: 'GET',
                    url: '{{ route('employee.vendors_company_get_products') }}',
                    data: {
                        id: id
                    },
                    success: function(res) {
                        ...
                        $.each(res, function(k, v) {
                            @if (LaravelLocalization::getCurrentLocale() == 'ar')
                                $('#product_id').append("<option value=" + v.id + " >" + v.name.ar +
                                    "<option>");
                            @else
                                $('#product_id').append("<option value=" + v.id + " >" + v.name.en +
                                    "<option>");
                            @endif
                        });
                    },
