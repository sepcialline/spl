    ...
    <body class="A5 landscape mx-auto" style="height: 100mm; width:100mm">
        ...
        <section class="sheet mx-auto p-2" style="width : 100%;height: 100%">
            ...
            <div class="d-flex justify-content-around">
                ...
                <div>
                    {!! $barcodes[$key] !!}
                    {{ $value['shipment']->shipment_no }}#{{ $value['shipment']->shipment_refrence }}
                </div>
                {{-- <div class="col">
            ...
            <div class="row d-flex justify-content-between mb-4">
                <div class="col-sm-5">
                    ...
                    <table>
                        <tbody>
                            <tr>
                                <td class="pe-3">{{ $value['shipment']->Company->name ?? '' }}</td>
                            </tr>
                            <tr>
                                <td class="pe-3">{{ $value['shipment']->Company->mobile ?? '' }}</td>
                            </tr>
                            <p class="mb-1">{{ $value['shipment']->Company->address ?? '' }} -
                                {{ $value['shipment']->Company->city->name ?? '' }} - {{ $value['shipment']->Company->emirate->name ?? '' }}
                            </p>
                            </tr>
                            <tr>
                ...
                <div class="col-sm-5">
                    <h6>{{ __('admin.to') }}</h6>
                    <p class="mb-1">{{ $value['shipment']->Client->name ?? '' }}</p>
                    <p class="mb-1">{{ $value['shipment']->Client->mobile ?? '' }}</p>
                    <p class="mb-1">{{ $value['shipment']->Client->address ?? '' }} - {{ $value['shipment']->city->name ?? '' }} -
                        {{ $value['shipment']->emirate->name ?? '' }}</p>
                </div>

                <div class="col-sm-2">
                    <p>{{ __('admin.total') }}</p>
                    <p class="mb-1">{{ $value['shipment']->rider_should_recive ?? '' }} {{__('admin.currency')}}</p>
                </div>
            ...
            <div class="table-responsive">
                <table class="table border-top m-0">
                    <thead>
                        ...
                        @if (count($value['shipment_content']) > 0)
                            <tr>
                                <th>#</th>
                                <th>{{ __('admin.item') }}</th>
                    ...
                    <tbody>
                        ...
                        @if (count($value['shipment_content']) > 0)
                        @php $i = 1; @endphp
                        @foreach ($value['shipment_content'] as $content)
                            <tr>
                                <td>{{ $i++ }}</td>
                ...
                <h6>{{ __('admin.note:') }}</h6>
                <span>{{ $value['shipment']->shipment_notes }}</span>
            </div>
<hr>
            <div class="row">
                <div class="col-12">
