<aside id="layout-menu" class="layout-menu menu-vertical menu bg-menu-theme">
    ...
    <ul class="menu-inner py-1">
        ...
        @if (auth()->user()->can('admin-Product-show-page') ||
                auth()->user()->can('admin-Product-details-showPage') ||
                auth()->user()->can('admin-Product-details-transfer') ||
                auth()->user()->can('admin-warehouse-report'))
            {{-- warehouse --}}
        ...
        @if (auth()->user()->can('admin-admin-Show-Page') ||
                auth()->user()->can('admin-rider-show-page') ||
                auth()->user()->can('admin-Customers-Show-Page') ||
                auth()->user()->can('admin-role-show-page') ||
                auth()->user()->can('admin-Employees-show-page'))
                ...
                <ul class="menu-sub">
                    ...
                    <li class="menu-item">
                        <a href="{{ route('admin.vendors_index') }}"
                            class="menu-link {{ Request::routeIs('admin.vendors_index') ? 'active' : '' }}">
                            <div data-i18n="{{ __('admin.vendors') }}">
                                {{ __('admin.vendors') }}</div>
                        </a>
                    </li>
                    <li class="menu-item">
                        <a href="{{ route('admin.services_company_index') }}"
                            class="menu-link {{ Request::routeIs('admin.services_company_index') ? 'active' : '' }}">
                            <div data-i18n="{{ __('admin.service_request') }}">
                                {{ __('admin.service_request') }}</div>
                        </a>
                    </li>
                    <li class="menu-item">
                        <a href="{{ route('admin.compensation_request_index') }}"
                            class="menu-link {{ Request::routeIs('admin.compensation_request_index') ? 'active' : '' }}">
                            <div data-i18n="{{ __('admin.compensation_request') }}">
                                {{ __('admin.compensation_request') }}</div>
                        </a>
                    </li>
        ...
        @if (auth()->user()->can('admin-Shipment-Report') ||
                auth()->user()->can('admin-Shipment-add') ||
                auth()->user()->can('admin-Shipment-daily-report') ||
                auth()->user()->can('admin-Shipment-assign rider') ||
                auth()->user()->can('admin-Shipment-print') ||
                auth()->user()->can('admin-Shipment-vendor-shipments'))
            <!-- shipments -->
            <li class="menu-item {{ request()->is('*/*/shipments/*') ? 'active open' : '' }}  ">
                <a href="javascript:void(0);" class="menu-link menu-toggle">
                    <i class="menu-icon tf-icons bx bxs-truck"></i>
                    <div data-i18n="{{ __('admin.shipments_shipments') }}">{{ __('admin.shipments_shipments') }}
                    </div>
                </a>
                <ul class="menu-sub">
        ...
        @if (auth()->user()->can('admin-Transaction-Collect-Rider-Cash') ||
                auth()->user()->can('admin-Transaction-Withdrwal-Bank') ||
                auth()->user()->can('admin-Transaction-Withdrawal-From-Merchant') ||
                auth()->user()->can('admin-Transaction-Pay-To-Merchant') ||
                auth()->user()->can('admin-Transaction-Show-Journals') ||
                auth()->user()->can('admin-Transaction-Journal-Voucher') ||
                auth()->user()->can('admin-Transaction-Recipt-Voucher') ||
                auth()->user()->can('admin-Transaction-Payment-Voucher'))
            <!-- Misc -->
                    ...
                    <li class="menu-item  {{ request()->is('*/*/account/*') ? '  active open' : '' }}">
                        ...
                        <ul class="menu-sub">
                            @can('admin-Transaction-Show-Journals')
                                ...
                                {{-- ميزان مراجعة  --}}
                                <li class="menu-item">
                                    <a href="{{ route('admin.account.getAccountReport') }}"
                                        class="menu-link {{ Request::routeIs('admin.account.getAccountReport') ? 'active' : '' }}">
                                        <div data-i18n="{{ __('admin.accounts_reports') }}">
                                            {{ __('admin.accounts_reports') }}</div>
                                    </a>
                                </li>
                                {{-- <li class="menu-item">
                                <a href="{{ route('admin.account.cacl') }}"
                                    class="menu-link {{ Request::routeIs('admin.account.cacl') ? 'active' : '' }}">
                                    <div data-i18n="{{ __('admin.calc') }}">
                                        {{ __('admin.calc') }}</div>
                                </a>
                            </li>  --}}
                            @endcan
                            ...
                            <a href="javascript:void(0);" class="menu-link menu-toggle">
                                <i class="menu-icon tf-icons bx bx-no-entry"></i>
                                <div data-i18n="{{ __('admin.account_final') }}">{{ __('admin.account_final') }}</div>
                            </a>
                            <ul class="menu-sub">
                                <li class="menu-item">
                                    <a href="{{ route('admin.account.budget') }}"
                                        class="menu-link {{ Request::routeIs('admin.account.budget') ? 'active' : '' }}">
                                        <div data-i18n="{{ __('admin.budget') }}">
                                            {{ __('admin.budget') }}</div>
                                    </a>
                                </li>
                                <li class="menu-item">
                                    <a href="{{ route('admin.account.profits_and_losses') }}"
                                        class="menu-link {{ Request::routeIs('admin.account.profits_and_losses') ? 'active' : '' }}">
                                        <div data-i18n="{{ __('admin.profits_and_losses') }}">
                                            {{ __('admin.profits_and_losses') }}</div>
                                    </a>
                                </li>
                            </ul>
        ...
        @if (auth()->user()->can('admin-Reports-payments') ||
            ...
            <li class="menu-item {{ request()->is('*/*/reports/*') ? '  active open' : '' }}">
                ...
                <ul class="menu-sub">
                    @can('admin-Reports-payments')
                        ...
                        <li class="menu-item">
                            <a href="{{ route('admin.companies_balance') }}"
                                class="menu-link {{ Request::routeIs('admin.companies_balance') ? 'active' : '' }}">
                                <div data-i18n="{{ __('admin.companies_balance') }}">
                                    {{ __('admin.companies_balance') }}</div>
                            </a>
                        </li>
                        {{-- <li class="menu-item">
        ...
        @if (auth()->user()->can('admin-Branch-Show-Page') ||
            ...
            @can('admin-Branch-Show-Page')
                ...
                <li class="menu-item {{ request()->is('*/*/branch/*') ? ' active ' : '' }}">
                    <a href="{{ route('admin.branch_index') }}" class="menu-link">
                        ...
                        <div data-i18n="{{ __('admin.branch_branch_manager') }}">
            ...
            @can('admin-City-Show-Page')
                ...
                <li class="menu-item {{ request()->is('*/*/cars') ? ' active ' : '' }}">
                    <a href="{{ route('admin.cars_index') }}" class="menu-link" target="">
                        ...
                        <div data-i18n="{{ __('admin.user_management_rider_vehicle_type') }}">
                            {{ __('admin.user_management_rider_vehicle_type') }}</div>
                    </a>
                </li>
            @endcan
